...
metaDescription: "Pull your TikTok data beyond the 60-day dashboard limit using the analytics API, with exports, third-party tools, and automated metrics tracking."
...
One limitation: TikTok's API does not expose earnings from the [Creator Rewards Program](https://support.tiktok.com/en/business-and-creator/creator-rewards-program/creator-rewards-program) (which replaced the Creativity Program beta and legacy Creator Fund) or from LIVE gifts. Revenue data stays locked inside the TikTok app. Creators who want to correlate earnings with performance metrics need to manually log their payouts or use screenshots.
...
- **Engagement rate per video** — (likes + comments + shares + saves) divided by views
...
These figures are typical of what approved developers report — TikTok adjusts exact quotas per app and over time, so always confirm current limits in the TikTok for Developers portal. Each API call returns up to 20 videos by default (configurable up to 100). A creator with 500 published videos needs at minimum 5 API calls to pull stats for their entire catalog — well within daily limits for any tier above Sandbox.
...
**Weekly review (15 minutes).** Pull the past 7 days of video data. Identify your top performer and worst performer. Note what differed — hook, topic, length, posting time. Check your [video performance metrics](/calculators/viral-potential/) against your 30-day averages.
...
### Does the TikTok API show Creator Rewards Program earnings?

No. TikTok does not expose revenue data through any public API endpoint. Earnings from the Creator Rewards Program (the current monetization program, which pays $0.50-$2.00 per 1,000 qualified views), the legacy Creator Fund, or LIVE gifts remain visible only inside the TikTok app and TikTok Studio. Creators who want to correlate earnings with video performance need to manually record payout data and match it against API-pulled metrics.
...
- [Video Performance Calculator](/calculators/viral-potential/) — Analyze individual video metrics against your historical averages
