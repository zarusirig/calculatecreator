...
interface ThinPageRecord {
  path: string;
  route: string;
  sourceType: 'app-page' | 'mdx';
  wordCount: number;
  hasUniqueIntro: boolean;
  hasFAQ: boolean;
  hasSchema: boolean;
  hasNoindex: boolean;
  sharedSimilarity: number;
  risk: 'high' | 'medium' | 'low';
  notes: string[];
}

interface PreliminaryRecord extends Omit<ThinPageRecord, 'risk' | 'sharedSimilarity'> {
  textBody: string;
}
...
const OUTPUT_PATH = path.join(ROOT, 'thin-pages-report.json');
...
function normalizeRouteFromAppFile(filePath: string): string {
...
function normalizeRouteFromMdx(filePath: string): string {
...
function stripTsxToText(raw: string): string {
...
function riskLevel(record: Omit<ThinPageRecord, 'risk' | 'sharedSimilarity'>, sharedSimilarity: number): 'high' | 'medium' | 'low' {
  const route = record.route;
  const isToolDetail =
    (route.startsWith('/calculators/') || route.startsWith('/tools/')) &&
    route !== '/calculators/' &&
    route !== '/tools/';
  ...
  if (isUtility && !record.hasNoindex) return 'high';
  if (isUtility && record.hasNoindex) return 'medium';
  if (isDirectory && record.wordCount < 500) return 'medium';
  if (isDirectory && sharedSimilarity >= 0.8) return 'medium';
  if (record.wordCount < 300) return 'high';
  if (sharedSimilarity >= 0.8) return 'high';
  if (isToolDetail && record.wordCount < 500) return 'high';
  if (record.wordCount < 500) return 'medium';
  if (isToolDetail && !record.hasFAQ) return 'medium';
  if (!record.hasUniqueIntro) return 'medium';
  return 'low';
}

function buildReport(): ThinPageRecord[] {
  const appPages = walk(APP_DIR)
    .filter((f) => f.endsWith('/page.tsx'))
    .filter((f) => !f.includes('/['));
  const mdxPages = walk(CONTENT_DIR).filter((f) => f.endsWith('.mdx'));

  const preliminary: PreliminaryRecord[] = [];

  for (const filePath of appPages) {
    const route = normalizeRouteFromAppFile(filePath);
    if (!shouldAuditRoute(route)) continue;
    ...
    const hasNoindex =
      /name=["']robots["'][^>]*noindex/i.test(sourceRaw) ||
      Boolean(html && /name="robots"[^>]*noindex/i.test(html));
    const hasSchema =
      /application\/ld\+json|FAQPageSchema|WebPageSchema|Organization|DefinedTermSet|BreadcrumbList/.test(sourceRaw) ||
  ...
  const report: ThinPageRecord[] = preliminary.map((record, index) => {
    let maxSimilarity = 0;
    ...
    const notes: string[] = [];
    if (record.wordCount < 500) notes.push('Below 500 words');
    if (!record.hasUniqueIntro) notes.push('Weak intro signal');
    ...
    if (!record.hasSchema) notes.push('No schema signal detected');
    ...
    const outputRecord: Omit<ThinPageRecord, 'risk' | 'sharedSimilarity'> = {
      path: record.path,
      route: record.route,
