...
// ── Helpers ──────────────────────────────────────────────────────────────────
const CONTENT_DIR = path_1.default.resolve(process.cwd(), "content");
/**
...
function parseArticle(filePath) {
    ...
    const fm = {
        ...
        parentArticle: data.parentArticle ?? "",
        ...
        siblingArticles: Array.isArray(data.siblingArticles)
            ...
            : [],
        parentCalculator: data.parentCalculator ?? "",
 ...
 */
function isLikelyCalculatorPage(targetPath, knownArticlePaths, calculatorPages, categoryPaths) {
    if (knownArticlePaths.has(targetPath))
        return false; // it's an article
    // Known parentCalculator values are explicitly calculator pages
    if (calculatorPages.has(targetPath))
        return true;
    ...
    if (segments[0] === "calculators" && segments.length <= 2)
        return true;
...
function main() {
    ...
    const articles = mdxFiles.map(parseArticle);
    // Build lookup structures
    const articleByPath = new Map();
    const knownArticlePaths = new Set();
    ...
    const orphans = articles.filter((a) => !a.frontmatter.parentArticle);
    // ── Section 2: Dead-End Articles ────────────────────────────────────────
    const deadEnds = articles.filter((a) => {
    ...
    for (const article of articles) {
        ...
        for (const ref of allRefs) {
            if (knownArticlePaths.has(ref))
                continue; // exists as article
            if (isLikelyCalculatorPage(ref, knownArticlePaths, calculatorPages, categoryPaths))
                continue;
            brokenLinks.push({ article: article.fullPath, target: ref });
        }
    }
    // ── Section 5: Calculator Coverage ──────────────────────────────────────
    const calcCoverage = new Map();
    for (const article of articles) {
    ...
    const densities = articles.map((a) => {
    ...
    const avgDensity = densities.length > 0 ? totalDensity / densities.length : 0;
    const maxEntry = densities.reduce((best, d) => (d.count > best.count ? d : best), densities[0] ?? { path: "(none)", count: 0 });
    const minEntry = densities.reduce((best, d) => (d.count < best.count ? d : best), densities[0] ?? { path: "(none)", count: 0 });
    // ── Output ──────────────────────────────────────────────────────────────
    const lines = [];
    ...
    const sortedCalcs = [...calcCoverage.entries()].sort((a, b) => a[0].localeCompare(b[0]));
    ...
    log();
    log("HUB COVERAGE:");
    const sortedHubs = [...hubCoverage.entries()].sort((a, b) => a[0].localeCompare(b[0]));
